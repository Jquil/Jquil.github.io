...
/****** Object:  StoredProcedure [dbo].[P_ESP_Sys_UserMenuList]    Script Date: 2022/8/15 16:01:36 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[P_ESP_Sys_UserMenuList]( 
	@userID NVARCHAR(50)
	)
AS
	SET NOCOUNT ON;
...
-- ModifyLog（修改记录）: 
-- Example（测试用例）: exec P_ESP_Sys_UserMenuList  '0776'
-- =============================================
*/

DECLARE @IsAdmin INT;
DECLARE @IsDevUser INT;

/*创建临时表*/
create table #AllMenu(
	UserId nvarchar(50) null,		/* 用户ID */
	ID nvarchar(50) null,			/* 菜单ID */
	MenuName nvarchar(150) null,		/* 菜单名称 */
	Sort nvarchar(50) null,			/* 菜单排序，父级归父级排序，子级归子级排序 */
	Url nvarchar(500) null,			/* 菜单地址 */
	Moduleflag nvarchar(150) null,
...
create table #MenuTab(
	...
	(
		...
		MenuGuid nvarchar(50) null,
		MenuName nvarchar(150) null
	)

	IF UPPER(@userID)  ='TYLD-DEV-ROLES'  --开发者账号
	BEGIN
	PRINT '开发者账号'
		SET @IsDevUser = 1;
	END
	...
	BEGIN
		...
		SELECT DISTINCT  @userID AS UserId, 
			md.ID,md.MenuName,md.Sort,md.Url,md.Moduleflag,md.Remark
	 ...
	 BEGIN
		...
		SELECT DISTINCT @userID
		 AS UserId, md.ID,md.MenuName,md.Sort,md.Url,md.Moduleflag,md.Remark
			 ...
			 LEFT JOIN  T_ESP_Sys_WorkflowProcessDefinition wpd ON md.WorkflowProcessdefinitionId = CAST(wpd.Id AS NVARCHAR(50))
			 LEFT JOIN T_ESP_Sys_QuickMenu qk ON qk.ChildMenu_GuID = md.Id
			  WHERE ISNULL(md.IsDelete,'0')='0' -- AND md.MenuName !='菜单管理'
	 END
	 ELSE
	 BEGIN
		/*普通用户根据配置的权限拿相应的菜单 */
	...
	IF @userID ='TYLD-DEV-ROLES'  --开发者账号
	BEGIN
		--SET @strSql ='';
		 -- SELECT *  INTO  #MenuTab  FROM #AllMenu WHERE  ISNULL(IsEnabled,'1') ='1' AND ISNULL(IsDelete,'0')='0' ;
		 INSERT INTO #MenuTab(UserId,ID,MenuName,Sort,Url,Moduleflag,Remark,ParentId,ParentName,WorkflowProcessdefinitionId,workflowProcessdefinitionName,IsEncrypt,IsShow,IsDelete,IsEnabled,ControlName)
		 SELECT UserId,ID,MenuName,Sort,Url,Moduleflag,Remark,ParentId,ParentName,WorkflowProcessdefinitionId,workflowProcessdefinitionName,IsEncrypt,IsShow,IsDelete,IsEnabled,ControlName
		 FROM #AllMenu WHERE  ISNULL(IsEnabled,'1') ='1' AND ISNULL(IsDelete,'0')='0' ;
    ...
    BEGIN
		INSERT INTO #MenuTab(UserId,ID,MenuName,Sort,Url,Moduleflag,Remark,ParentId,ParentName,WorkflowProcessdefinitionId,workflowProcessdefinitionName,IsEncrypt,IsShow,IsDelete,IsEnabled,ControlName)
		SELECT UserId,ID,MenuName,Sort,Url,Moduleflag,Remark,ParentId,ParentName,WorkflowProcessdefinitionId,workflowProcessdefinitionName,IsEncrypt,IsShow,IsDelete,IsEnabled,ControlName
		FROM #AllMenu WHERE  ISNULL(IsEnabled,'1') ='1' AND ISNULL(IsDelete,'0')='0' AND  Userid = @userID ;
	END;



	--  exec P_ESP_Sys_UserMenuList_test  '321'
	...
	*/
	...
	IF(@IsAdmin = 0 AND @IsDevUser = 0 )
	BEGIN	
		/* 菜单下的功能定义 */
		INSERT INTO #UserQueryPoweTab(UserId,Id ,FeatureName,FeatureID, MenuGuid,md.MenuName)
	...
	BEGIN
		INSERT INTO #UserQueryPoweTab(UserId,Id ,FeatureName,FeatureID, MenuGuid,md.MenuName)
		SELECT DISTINCT @userID AS UserId, fd.Id ,fd.FeatureName,fd.FeatureID, md.ID AS 'MenuGuid',md.MenuName		
			FROM T_ESP_Sys_Menu md
			LEFT JOIN  T_ESP_Sys_Menu mm ON md.ParentId =CAST(mm.ID AS NVARCHAR(50))
			LEFT JOIN  T_ESP_Sys_WorkflowProcessDefinition wpd ON md.WorkflowProcessdefinitionId = CAST(wpd.Id AS NVARCHAR(50))
			LEFT JOIN  T_ESP_Sys_QuickMenu qk ON qk.ChildMenu_GuID = md.Id
