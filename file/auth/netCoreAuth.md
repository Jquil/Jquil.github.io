...
通过接口访问即可获得到Tooken令牌，我们再来新建一个控制器：
```
namespace WebApplication1.Controllers
{
    [Route("api/[Controller]/[Action]")]
...
因此我们需要请求授权接口拿到Tooken后，在请求头上加上Tooken才能访问接口：
<div align='center'>
...
上面我们设置了Tooken过期时间为30分钟，即表示30分钟后我们再次请求接口就会报401；对于Tooken过期的处理，可以有以下处理方式：

【1】让用户重新登陆，获取新的Tooken（适用于过期时间较长的情况）；

【2】前端进行响应拦截，若发现状态码为401（已有Tooken的情况下），则是Tooken过期，我们可以刷新Tooken，重新发起请求；
